...
const (
	...
	btpOperatorConfigmap = "test-configmap"

	adminAPIAccessSecret  = "test-secret"
	adminAPIAccessBaseURL = "test-baseurl"
	adminAPIAccessUAA     = `{
		"url": "test-url",
...
var _ = BeforeSuite(func() {
	...
	err = hanav1.AddToScheme(scheme.Scheme)
	Expect(err).NotTo(HaveOccurred())
	...
	Expect(k8sClient.Create(context.Background(), &corev1.Secret{
		...
		Data: map[string][]byte{
			"baseurl": []byte(adminAPIAccessBaseURL),
			"uaa":     []byte(adminAPIAccessUAA),
		},
	})).To(Succeed())
})
